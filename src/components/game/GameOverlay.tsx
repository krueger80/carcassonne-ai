        ...
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden', userSelect: 'none', WebkitUserSelect: 'none' }}>
            ...
            <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column', padding: '24px 12px', zIndex: 50, pointerEvents: 'none' }}>
                <div style={{ position: 'relative', pointerEvents: 'auto', alignSelf: 'flex-start', marginBottom: 20 }}>
                    <button aria-label={t('menu.menu', 'Menu')} aria-expanded={isMenuOpen} aria-controls="main-menu-dropdown" onClick={(e) => { e.stopPropagation(); setIsMenuOpen(!isMenuOpen) }} style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid #555', borderRadius: 8, color: 'white', padding: 8, cursor: 'pointer' }}>
                    ...
                    <AnimatePresence>
                        {isMenuOpen && (
                            <motion.div id="main-menu-dropdown" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} style={{ position: 'absolute', top: '100%', left: 0, background: 'rgba(30, 30, 40, 0.95)', border: '1px solid #555', borderRadius: 12, padding: 12, display: 'flex', flexDirection: 'column', gap: 8, minWidth: 220, pointerEvents: 'auto' }}>
